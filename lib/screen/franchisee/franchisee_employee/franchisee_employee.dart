...
class EmployeePageState extends State<EmployeePage> {
  late AppDatabase db;
  late StreamSubscription<List<User>> usersSub;
  late StreamSubscription<List<Role>> rolesSub;

  List<User> users = [];
  List<Role> roles = [];
  int? currentOrganizationId; // Store current user's organization
  ...
  void setupSubscriptions() {
    ...
    if (currentOrganizationId != null) {
      usersSub = db.usersDao.watchUsersByOrganization(currentOrganizationId!).listen((userList) {
        if (mounted) {
          setState(() {
            users = userList;
            isLoading = false;
