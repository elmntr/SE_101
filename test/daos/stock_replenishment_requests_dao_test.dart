...
void main() {
  ...
  setUp(() async {
    ...
    final roleId = await db.rolesDao.insertRole(
      RolesCompanion.insert(name: 'Franchisee Owner'),
    );
    final reviewerRoleId = await db.rolesDao.insertRole(
      RolesCompanion.insert(name: 'Commissary Staff'),
    );

    requesterId = await db.usersDao.insertUser(
    ...
    reviewerId = await db.usersDao.insertUser(
      UsersCompanion.insert(
        username: 'rev',
        email: 'v@v.com',
        password: 'pw',
  ...
  test('1. Create request starts in pending status', () async {
    final id = await dao.createRequest(
  ...
  test('5. Get pending requests for commissary returns correct list', () async {
    final id1 = await dao.createRequest(
      franchiseeId: franchiseeId,
      commissaryId: commissaryId,
      itemId: itemId,
      quantityRequested: 10,
    ...
    final id2 = await dao.createRequest(
      franchiseeId: franchiseeId,
      commissaryId: commissaryId,
      itemId: itemId,
  ...
  test(
    '6. Get approved requests awaiting delivery returns correct list',
    () async {
      ...
      final approved = await dao.getApprovedRequestsAwaitingDelivery(
        commissaryId,
      );
  ...
  test(
    '7. Get franchisee request history returns all their requests',
    () async {
      final otherFranchiseeId = await db.organizationsDao.insertOrganization(
        OrganizationsCompanion.insert(
          name: 'Other',
          type: 'franchisee',
          parentCommissaryId: Value(commissaryId),
  ...
  test('12. Get request count works with commissary filter', () async {
    final otherCommissaryId = await db.organizationsDao.insertOrganization(
      OrganizationsCompanion.insert(name: 'Other Comm', type: 'commissary'),
    );
    await dao.createRequest(
      franchiseeId: franchiseeId,
      commissaryId: commissaryId,
    ...
    await dao.createRequest(
      franchiseeId: franchiseeId,
      commissaryId: otherCommissaryId,
      itemId: itemId,
      quantityRequested: 20,
      requestedBy: requesterId,
    );
  ...
  test('15. Sort order works correctly (oldest first)', () async {
    ...
    final requests = await dao.getAllRequests(
      sortOrder: RequestSortOrder.oldestFirst,
    );
