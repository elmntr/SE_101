...
void main() {
  ...
  UsersCompanion createUser(String username, String email) {
  ...
  test('1. Insert user successfully hashes password', () async {
    final id = await usersDao.insertUser(
      createUser('testuser', 'test@test.com'),
    );
  ...
  test('2. Get all users returns inserted users', () async {
    await usersDao.insertUser(createUser('user1', 'user1@test.com'));
    await usersDao.insertUser(createUser('user2', 'user2@test.com'));
    final allUsers = await usersDao.getAllUsers();
    expect(allUsers.length, 2);
  });

  test('3. Update user changes their properties', () async {
    final id = await usersDao.insertUser(createUser('olduser', 'old@test.com'));
    final user = (await usersDao.getUserById(id))!;
  ...
  test('4. Deactivate user marks them as inactive', () async {
    final id = await usersDao.insertUser(
      createUser('activeuser', 'active@test.com'),
    );
  ...
  test('5. Get all users with isActive=true ignores inactive users', () async {
    await usersDao.insertUser(createUser('userA', 'a@test.com'));
    final idB = await usersDao.insertUser(createUser('userB', 'b@test.com'));
  ...
  test('6. Get user by username finds the correct user', () async {
    await usersDao.insertUser(createUser('findme', 'find@me.com'));
  ...
  test('8. Authenticate user succeeds with correct credentials', () async {
    await usersDao.insertUser(createUser('authuser', 'auth@test.com'));
  ...
  test('10. Authenticate user fails for inactive user', () async {
    final id = await usersDao.insertUser(
      createUser('authuser', 'auth@test.com'),
    );
    await usersDao.deactivateUser(id);
    final user = await usersDao.authenticate('auth@test.com', 'password123');
    expect(user, isNull);
  ...
  test('11. Update password changes the stored hash', () async {
    final id = await usersDao.insertUser(
      createUser('pwchange', 'pw@change.com'),
    );
    ...
    final authenticated = await usersDao.authenticate(
      'pw@change.com',
      'newpassword',
    );
  ...
  test('12. Get users with roles includes role data', () async {
    await usersDao.insertUser(createUser('userwithrole', 'role@test.com'));
  ...
  test('13. Assign role to user updates the user record', () async {
    final newRoleId = await db.rolesDao.insertRole(
      RolesCompanion.insert(name: 'New Role'),
    );
    final userId = await usersDao.insertUser(
      createUser('assignrole', 'assign@role.com'),
    );
  ...
  test('14. Delete user permanently removes them', () async {
    final id = await usersDao.insertUser(
      createUser('todelete', 'delete@me.com'),
    );
  ...
  test('16. Get unsynced user count is correct', () async {
    final id1 = await usersDao.insertUser(
      createUser('user1', 'user1@test.com'),
    );
    final id2 = await usersDao.insertUser(
