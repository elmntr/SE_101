...
void main() {
  ...
  test('2. Insert franchisee successfully', () async {
    ...
    final franchiseeId = await organizationsDao.insertOrganization(
      OrganizationsCompanion.insert(name: 'Branch A', type: 'franchisee', parentCommissaryId: Value(commissaryId)),
    );
  ...
  test('4. Get all organizations returns correct count', () async {
    final c1Id = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Com1', type: 'commissary'));
    await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Fra1', type: 'franchisee', parentCommissaryId: Value(c1Id)));
  ...
  test('5. Update organization changes its properties', () async {
    final id = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Old Name', type: 'commissary'));
    final org = (await organizationsDao.getOrganizationById(id))!;
  ...
  test('6. Deactivate organization marks it as inactive', () async {
    final id = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'To Deactivate', type: 'commissary'));
  ...
  test('7. Get all organizations ignores inactive ones by default', () async {
    await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Active', type: 'commissary'));
    final id = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Inactive', type: 'commissary'));
    await organizationsDao.deactivateOrganization(id);
  ...
  test('8. Reactivate organization marks it as active', () async {
    final id = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'To Reactivate', type: 'commissary'));
    await organizationsDao.deactivateOrganization(id);
  ...
  test('10. Get all franchisees returns only franchisees', () async {
    final cId = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Com1', type: 'commissary'));
    await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Fra1', type: 'franchisee', parentCommissaryId: Value(cId)));
  ...
  test('11. Get franchisees by commissary filters correctly', () async {
    final c1Id = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Com1', type: 'commissary'));
    final c2Id = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Com2', type: 'commissary'));
    await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Fra1', type: 'franchisee', parentCommissaryId: Value(c1Id)));
    await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Fra2', type: 'franchisee', parentCommissaryId: Value(c2Id)));

    final c1Franchisees = await organizationsDao.getFranchiseesByCommissary(c1Id);
  ...
  test('12. Deactivate fails if organization has active users', () async {
    final cId = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Com', type: 'commissary'));
    final orgId = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Org With User', type: 'franchisee', parentCommissaryId: Value(cId)));
    final roleId = await db.rolesDao.insertRole(RolesCompanion.insert(name: 'Test Role'));
    ...
    expect(() => organizationsDao.deactivateOrganization(orgId), throwsException);
  });

  test('13. Deactivate fails if organization has active items', () async {
    final cId = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Com', type: 'commissary'));
    final orgId = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Org With Item', type: 'franchisee', parentCommissaryId: Value(cId)));
  ...
  test('14. Get organization count works with filters', () async {
    final cId = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Com1', type: 'commissary'));
    await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Fra1', type: 'franchisee', parentCommissaryId: Value(cId)));
    await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Fra2', type: 'franchisee', parentCommissaryId: Value(cId)));

    final count = await organizationsDao.getOrganizationCount(type: 'franchisee');
    expect(count, 2);
  });

  test('15. Get main commissary returns the first created one', () async {
    await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Second Commissary', type: 'commissary'));
    await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'First Commissary', type: 'commissary'));
    ...
    final firstId = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'First', type: 'commissary'));
    await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'Second', type: 'commissary'));
    
    final mainAfterReset = await organizationsDao.getMainCommissary();
    expect(mainAfterReset!.id, firstId);
  });

   test('16. Get organization by name finds the correct active organization', () async {
    await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'FindMe', type: 'commissary'));
    final id = await organizationsDao.insertOrganization(OrganizationsCompanion.insert(name: 'FindMeButInactive', type: 'commissary'));
    await organizationsDao.deactivateOrganization(id);
