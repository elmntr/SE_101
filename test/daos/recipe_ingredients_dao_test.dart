...
void main() {
  ...
  setUp(() async {
    ...
    ingredient1Id = await db.ingredientsDao.insertIngredient(
      name: 'Bun',
      commissaryId: commissaryId,
      stock: 100,
    );
    ingredient2Id = await db.ingredientsDao.insertIngredient(
      name: 'Patty',
      commissaryId: commissaryId,
      stock: 50,
    );
  ...
  test('2. Get ingredients for item returns correct list', () async {
    await recipeIngredientsDao.insertRecipeIngredient(
      itemId: itemId,
      ingredientId: ingredient1Id,
      quantityNeeded: 2,
      unit: 'pieces',
    ...
    final ingredients = await recipeIngredientsDao.getIngredientsForItem(
      itemId,
    );
    expect(ingredients.length, 2);
    expect(
      ingredients.map((ri) => ri.ingredientId),
      containsAll([ingredient1Id, ingredient2Id]),
    );
  });
  ...
  test('3. Update recipe ingredient changes its quantity', () async {
    ...
    final recipe = (await recipeIngredientsDao.getRecipeIngredientById(id))!;
    await recipeIngredientsDao.updateRecipeIngredient(
      recipe.copyWith(quantityNeeded: 3.0),
    );
  ...
  test('5. Get ingredients for item ignores soft-deleted ones', () async {
    ...
    final idToDelete = await recipeIngredientsDao.insertRecipeIngredient(
      itemId: itemId,
      ingredientId: ingredient2Id,
      quantityNeeded: 1,
      unit: 'piece',
  ...
  test('6. Get items using ingredient returns correct list', () async {
    ...
    final items = await recipeIngredientsDao.getItemsUsingIngredient(
      ingredient1Id,
    );
  ...
  test(
    '7. Delete all for item soft-deletes all ingredients for that recipe',
    () async {
      await recipeIngredientsDao.insertRecipeIngredient(
        itemId: itemId,
        ingredientId: ingredient1Id,
  ...
  test('8. Calculate ingredient requirements works correctly', () async {
    ...
    final requirements = await recipeIngredientsDao
        .calculateIngredientRequirements(itemId, 10);
  ...
  test(
    '10. Can produce items returns false when stock is insufficient',
    () async {
      await recipeIngredientsDao.insertRecipeIngredient(
        itemId: itemId,
        ingredientId: ingredient1Id,
  ...
  test(
    '11. Deduct ingredients for production updates ingredient stock',
    () async {
      await recipeIngredientsDao.insertRecipeIngredient(
        itemId: itemId,
        ingredientId: ingredient1Id,
  ...
  test(
    '12. Deduct ingredients for production fails if stock is insufficient',
    () async {
      await recipeIngredientsDao.insertRecipeIngredient(
        itemId: itemId,
        ingredientId: ingredient1Id,
      ...
      final success = await recipeIngredientsDao.deductIngredientsForProduction(
        itemId,
        1,
      );
  ...
  test('15. Get unsynced count is correct', () async {
    final id1 = await recipeIngredientsDao.insertRecipeIngredient(
      itemId: itemId,
      ingredientId: ingredient1Id,
      quantityNeeded: 1,
      unit: 'a',
