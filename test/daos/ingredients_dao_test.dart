...
void main() {
  ...
  test('1. Insert ingredient successfully', () async {
    final id = await ingredientsDao.insertIngredient(
      name: 'Flour',
  ...
  test('2. Get all ingredients returns inserted ingredients', () async {
    await ingredientsDao.insertIngredient(
      name: 'Sugar',
      commissaryId: commissaryId,
    );
    await ingredientsDao.insertIngredient(
  ...
  test('3. Update ingredient changes its properties', () async {
    ...
    final ingredient = (await ingredientsDao.getIngredientById(id))!;
    await ingredientsDao.updateIngredient(
      ingredient.copyWith(name: 'Margarine', stock: 100),
    );
  ...
  test('5. Get all ingredients ignores soft-deleted ones', () async {
    ...
    final idToDelete = await ingredientsDao.insertIngredient(
      name: 'Invisible',
      commissaryId: commissaryId,
    );
  ...
  test('12. Soft delete fails if ingredient is used in a recipe', () async {
    ...
    final ingredientId = await ingredientsDao.insertIngredient(
      name: 'Flour',
      commissaryId: commissaryId,
    );
    await db.recipeIngredientsDao.insertRecipeIngredient(
      itemId: itemId,
    ...
    expect(
      () => ingredientsDao.softDeleteIngredient(ingredientId),
      throwsException,
    );
  });
  ...
  test('13. Get ingredient by name finds the correct ingredient', () async {
    ...
    final ingredient = await ingredientsDao.getIngredientByName(
      'FindMe',
      commissaryId: commissaryId,
    );
    expect(ingredient, isNotNull);
  ...
  test('14. Filter by category ID in getAllIngredients works', () async {
    ...
    final dairyItems = await ingredientsDao.getAllIngredients(
      categoryId: cat1Id,
    );
  ...
  test('15. Sort order in getAllIngredients works (by stock desc)', () async {
    ...
    final sorted = await ingredientsDao.getAllIngredients(
      sortOrder: IngredientSortOrder.stockDesc,
    );
